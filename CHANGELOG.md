...
* Fixed asynchronous access to list of cancelled tasks to skip callback from in iOS downloading tasks restart for background mode.
* Fixed deleting cancelled and not valid downloading tasks in iOS background mode.
* Fixed crash in `olp::cache::DiskCache::Size()` method when called on closed cache.
...
* Improved iOS background downloading mode change. No additional actions performed when requested mode is already active.
* Improved cURL certificates configuration. It is allowed to setup each field independently.
