...
TEST_F(DefaultCacheImplTest, ProtectTest) {
  ...
  /*  {
      ...
      olp::cache::CacheSettings settings;
      settings.disk_path_mutable = olp::utils::Dir::TempDirectory() +
    "/unittest"; settings.max_memory_cache_size = 0; DefaultCacheImplHelper
    cache(settings); ASSERT_EQ(olp::cache::DefaultCache::Success, cache.Open());
    ...
    }
    {
      ...
      olp::cache::CacheSettings settings;
      settings.disk_path_mutable = olp::utils::Dir::TempDirectory() +
    "/unittest"; DefaultCacheImplHelper cache(settings);
...
TEST_F(DefaultCacheImplTest, LruCacheEvictionWithProtected) {
  {
    ...
    for (; count < max_count; ++count) {
      ...
      EXPECT_TRUE(cache.ContainsMutableCache(key));
      EXPECT_TRUE(cache.ContainsMemoryCache(key));
      //    EXPECT_FALSE(cache.ContainsLru(key));
    }
