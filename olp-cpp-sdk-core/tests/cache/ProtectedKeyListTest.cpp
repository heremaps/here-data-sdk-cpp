...
TEST(ProtectedKeyList, CanBeMoved) {
  ...
  EXPECT_EQ(list_a.GetWrittenListSize(), 0);
  EXPECT_TRUE(list_a.IsListDirty());
  auto raw_data = list_a.Serialize();
  EXPECT_TRUE(raw_data.get());
  EXPECT_EQ(raw_data->size(), 4);
  EXPECT_FALSE(list_a.IsListDirty());
  EXPECT_EQ(list_a.GetWrittenListSize(), raw_data->size());
  cache::ProtectedKeyList list_b(std::move(list_a));
  EXPECT_EQ(list_b.GetWrittenListSize(), 4);
  cache::ProtectedKeyList list_c;
  EXPECT_EQ(list_c.GetWrittenListSize(), 0);
  list_c = std::move(list_b);
  EXPECT_EQ(list_c.GetWrittenListSize(), 4);
}
...
TEST(ProtectedKeyList, Protect) {
  {
    ...
    auto cb = [](const std::string&) {};
    {
      ...
      EXPECT_FALSE(protected_keys.IsListDirty());

      // protect key
      EXPECT_TRUE(protected_keys.Protect({"key:1"}, cb));
      EXPECT_TRUE(protected_keys.IsListDirty());
      auto raw_data = protected_keys.Serialize();
      EXPECT_TRUE(raw_data.get());
      EXPECT_EQ(raw_data->size(), 6);
      EXPECT_FALSE(protected_keys.IsListDirty());
      EXPECT_EQ(protected_keys.GetWrittenListSize(), raw_data->size());
    }
