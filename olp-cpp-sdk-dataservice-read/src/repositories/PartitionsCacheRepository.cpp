...
client::ApiNoResponse PartitionsCacheRepository::Put(
  ...
  for (const auto& partition : partitions_list) {
    const auto key = cache::CacheKeyGenerator::CreatePartitionKey(
  ...
  if (layer_metadata) {
    const auto key = cache::CacheKeyGenerator::CreatePartitionsKey(
        catalog_, layer_id_, version);
    OLP_SDK_LOG_DEBUG_F(kLogTag, "Put -> '%s'", key.c_str());
...
void PartitionsCacheRepository::Put(
  ...
  const auto key = cache::CacheKeyGenerator::CreateLayerVersionsKey(
      catalog_, catalog_version);
  OLP_SDK_LOG_DEBUG_F(kLogTag, "Put -> '%s'", key.c_str());
...
client::ApiNoResponse PartitionsCacheRepository::Put(
    ...
    const boost::optional<int64_t>& version) {
  const auto key = cache::CacheKeyGenerator::CreateQuadTreeKey(
...
bool PartitionsCacheRepository::ContainsTree(
  ...
  return cache_->Contains(cache::CacheKeyGenerator::CreateQuadTreeKey(
...
PartitionsCacheRepository::CreatePartitionKeys(
  ...
  if (GetPartitionHandle(partition_id, version, handle)) {
    return cache::KeyValueCache::KeyListType{
        cache::CacheKeyGenerator::CreatePartitionKey(catalog_, layer_id_,
                                                     partition_id, version),
        cache::CacheKeyGenerator::CreateDataHandleKey(catalog_, layer_id_,
                                                      handle)};
  }
