...
const std::string cache_key = catalog + "::latestVersion";
const std::string service_name = "metadata";
const std::string service_version = "v1";
...
const auto hrn = olp::client::HRN::FromString(catalog);

struct CatalogRepositoryTest : public ::testing::Test {
  void SetUp() override {
    cache = std::make_shared<CacheMock>();
    network = std::make_shared<NetworkMock>();
  ...
  std::shared_ptr<CacheMock> cache;
...
TEST_F(CatalogRepositoryTest, GetLatestVersionCacheOnlyNotFound) {
  ...
  EXPECT_CALL(*cache, Get(_, _)).Times(1).WillOnce(Return(boost::any{}));
...
TEST_F(CatalogRepositoryTest, GetLatestVersionOnlineOnlyNotFound) {
  ...
  EXPECT_CALL(*network, Send(IsGetRequest(URL_LOOKUP_METADATA), _, _, _, _))
      .Times(1)
      .WillOnce(NetworkMock::ReturnHttpResponse(
          olp::http::NetworkResponse().WithStatus(404), ""));
...
TEST_F(CatalogRepositoryTest, GetLatestVersionOnlineOnlyFoundAndCacheWritten) {
  ...
  EXPECT_CALL(*cache,
              Put(Eq("hrn:here:data:::hereos-internal-test-v2::latestVersion"),
                  _, _, _))
      .Times(1);

  EXPECT_CALL(
      *cache,
      Put(Eq("hrn:here:data:::hereos-internal-test-v2::metadata::v1::api"), _,
          _, _))
      .Times(1);
...
TEST_F(CatalogRepositoryTest, GetLatestVersionOnlineOnlyUserCancelled1) {
  ...
  ON_CALL(*network, Send(IsGetRequest(URL_LOOKUP_METADATA), _, _, _, _))
      .WillByDefault(
          [&context](olp::http::NetworkRequest network,
  ...
  ON_CALL(*network, Send(IsGetRequest(URL_LATEST_CATALOG_VERSION), _, _, _, _))
      .WillByDefault([](olp::http::NetworkRequest network,
                        olp::http::Network::Payload payload,
                        olp::http::Network::Callback callback,
                        olp::http::Network::HeaderCallback header_callback,
                        olp::http::Network::DataCallback data_callback) {
