...
TEST_F(StreamLayerClientTest, PublishDataGreaterThanTwentyMib) {
  ...
  EXPECT_CALL(*network_, Send(IsGetRequest(URL_GET_CATALOG), _, _, _, _))
      .Times(1);
  EXPECT_CALL(*network_, Send(IsGetRequest(URL_LOOKUP_PUBLISH_V2), _, _, _, _))
      .Times(1);
  EXPECT_CALL(*network_, Send(IsPostRequest(URL_INIT_PUBLICATION), _, _, _, _))
      .Times(1);
  ...
  EXPECT_CALL(*network_,
      ...
      .Times(1);
  EXPECT_CALL(*network_, Send(IsPostRequest(URL_UPLOAD_PARTITIONS), _, _, _, _))
      .Times(1);
  EXPECT_CALL(*network_, Send(IsPutRequest(URL_SUBMIT_PUBLICATION), _, _, _, _))
      .Times(1);
...
TEST_F(StreamLayerClientTest, PublishDataCancel) {
  ...
  std::tie(request_id, send_mock, cancel_mock) = GenerateNetworkMockActions(
      wait_for_cancel, pause_for_cancel, {200, HTTP_RESPONSE_LOOKUP_INGEST});
  EXPECT_CALL(*network_, Send(IsGetRequest(URL_LOOKUP_CONFIG), _, _, _, _))
      .Times(1);
  EXPECT_CALL(*network_, Send(IsGetRequest(URL_GET_CATALOG), _, _, _, _))
      .Times(1);
