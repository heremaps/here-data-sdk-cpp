...
class StreamLayerClientMockTest : public StreamLayerClientTestBase {
 protected:
  ...
  void SetUpCommonNetworkMockCalls(NetworkMock& network) {
    ...
    ON_CALL(network,
            Send(testing::AnyOf(IsPostRequest(URL_UPLOAD_PARTITIONS),
...
TEST_P(StreamLayerClientMockTest, DISABLED_PublishData) {
  {
    ...
    EXPECT_CALL(*network_,
                Send(IsGetRequest(URL_LOOKUP_INGEST), _, _, _, _))
        .Times(1);
    EXPECT_CALL(*network_,
...
TEST_P(StreamLayerClientMockTest, DISABLED_BillingTag) {
  {
    ...
    EXPECT_CALL(*network_,
        ...
        .Times(1);
    EXPECT_CALL(*network_, Send(IsPostRequest(URL_INGEST_DATA_BILLING_TAG),
                                _, _, _, _))
        .Times(1);
  }
...
TEST_P(StreamLayerClientMockTest, DISABLED_PublishSDIIBillingTag) {
  {
    ...
    EXPECT_CALL(*network_,
                Send(IsGetRequest(URL_LOOKUP_CONFIG), _, _, _, _))
        .Times(1);
    EXPECT_CALL(*network_, Send(IsPostRequest(URL_INGEST_SDII_BILLING_TAG),
                                _, _, _, _))
        .Times(1);
  }
...
TEST_P(StreamLayerClientMockTest, DISABLED_SDIIConcurrentPublishSameIngestApi) {
  {
    ...
    EXPECT_CALL(*network_,
        ...
        .Times(1);
    EXPECT_CALL(*network_,
                Send(IsPostRequest(URL_INGEST_SDII), _, _, _, _))
        .Times(6);
  }
...
TEST_P(StreamLayerClientCacheMockTest,
       DISABLED_FlushDataMaxEventsDefaultSetting) {
  {
    testing::InSequence dummy;
